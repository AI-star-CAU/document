...
| `turn.token_count` | FR-5.2 긴 대화 처리, FR-5.3 토큰 사용량 표시. |
...
2. 분기 제목 생성
3. `chat` row 업데이트 — `title`, `title_status = GENERATED`
...
복구 가능 기간(예: 30일) 안에만 가능. 만료 시 `BRANCH_4092`를 반환한다. 정확한 기간 정책은 별도 결정.
...
```http
GET /api/v1/chats/{chatId}/parent
Authorization: Bearer <accessToken>
```

**인증 필요:** ✓

현재 chat의 부모 chat 및 분기점 turn 정보를 반환한다. 이동 자체는 클라이언트에서 처리하며, 본 엔드포인트는 이동 대상의 좌표를 알려주는 역할이다.

> 클라이언트는 응답을 받아 `GET /chats/{chatId}/graph?center={branchPointTurnId}`을 호출하면 부모 분기의 분기점으로 자연스럽게 이동한다.
...
| 필드 | 타입 | 설명 |
|---|---|---|
| `parentChatId` | Long? | 부모 chat의 ID. 루트면 null |
| `branchPointTurnId` | Long? | 분기점 turn. 루트면 null |
| `rootChatId` | Long | 분기 트리 루트 |
...
```http
GET /api/v1/chats/{chatId}/graph?center={turnId}&up=30&down=30
Authorization: Bearer <accessToken>
```

**인증 필요:** ✓

지정한 root chat의 분기 구조를 반환한다. chat 골격은 전체 반환되고, turn 노드는 `center` 기준 window만 포함된다.
...
| 파라미터 | 타입 | 기본값 | 설명 |
|---|---|---|---|
| `center` | Long? | path의 chatId에 해당하는 chat의 `lastTurnId`, 없으면 root chat의 `lastTurnId` | window 중심 turn. 클라이언트가 명시 전달 권장 |
| `up` | Integer | 30 | center에서 ancestor 방향 최대 홉 수 (1~100) |
| `down` | Integer | 30 | center에서 자손 방향 BFS 최대 홉 수 (1~100) |
...
```json
{
  ...
  "result": {
    ...
    "frontier": {
      "up":   [ { "fromTurnId": 103, "hasMore": true  } ],
...
| HTTP | code | 시점 |
|---|---|---|
| 400 | `GRAPH_4001` | up/down 범위 위반, center가 chatId의 트리에 속하지 않음 |
| 401 | `AUTH_4011` | |
| 403 | `COMMON_403` | |
| 404 | `CHAT_4041` | 존재하지 않는 chatId |
| 404 | `TURN_4041` | 존재하지 않는 center turn |
...
```http
GET /api/v1/chats/{chatId}/graph/expand?fromTurnId={id}&direction=UP&limit=30
Authorization: Bearer <accessToken>
```
...
| HTTP | code | 시점 |
|---|---|---|
| 400 | `GRAPH_4001` | 잘못된 direction, limit 범위 위반 |
| 401 | `AUTH_4011` | |
| 403 | `COMMON_403` | |
...
3. 부모 turn(`turn_sequence - 1`)을 분기점으로 새 분기 자동 생성
4. 새 분기 안에서 user 메시지 재기록 + LLM 응답 스트리밍
...
data: {"aiMessageId": 401, "summary": "...", "answerToken": 350}

event: done
data: {}
```

**SSE 이벤트 타입 (Phase 2 §2.5 대비 추가)**

| event | data 스키마 | 의미 |
|---|---|---|
| `branch_created` | `{newChatId: Long, branchPointTurnId: Long, title: String, titleStatus: Enum}` | 분기 자동 생성 알림. 클라이언트는 그래프 갱신 |
| 이하 | (Phase 2와 동일) | |
...
| 409 | `MESSAGE_4092` | senderType이 ASSISTANT가 아니거나 STREAMING 상태 |
...
과거 user 메시지를 수정한다. 기존 메시지는 보존되며, 해당 메시지가 속한 turn의 직전 turn을 분기점으로 새 분기가 자동 생성되고 그 안에서 수정된 메시지로 첫 응답이 스트리밍된다.
...
| 404 | `CHAT_4041` | |
| 404 | `MESSAGE_4041` | |
| 409 | `MESSAGE_4092` | senderType이 USER가 아님 |
| stream | `LLM_5001` | |
...
2. 프론트가 그래프 갱신을 위해 재조회
   → GET /api/v1/chats/42/graph?center=105
...
5. 다음 graph 조회 시 갱신된 제목 반영
   → GET /api/v1/chats/42/graph?center=200
```
...
1. 사용자가 그래프에서 다른 분기의 노드 turn 300 클릭
   → GET /api/v1/chats/42/graph?center=300
...
|---|---|---|---|---|
| POST | `/api/v1/chats/{chatId}/branches` | 분기 생성 | ✓ | FR-2.1, FR-2.2 |
| PATCH | `/api/v1/chats/{chatId}` | 분기 제목 수정 | ✓ | FR-2.2 |
| DELETE | `/api/v1/chats/{chatId}` | 분기 삭제 (cascade) | ✓ | FR-2.5 |
| POST | `/api/v1/chats/{chatId}/restore` | 분기 복구 | ✓ | FR-2.5 |
| GET | `/api/v1/chats/{chatId}/parent` | 부모 분기 정보 | ✓ | FR-2.4 |
| GET | `/api/v1/chats/{chatId}/graph` | 그래프 조회 (window) | ✓ | FR-3.1, FR-3.3, FR-3.4 |
| GET | `/api/v1/chats/{chatId}/graph/expand` | 윈도우 확장 | ✓ | FR-3.2, FR-3.5 |
| POST | `/api/v1/chats/{chatId}/messages/{messageId}/regenerate` | 응답 재생성 (자동 분기) | ✓ | FR-1.4 |
| PATCH | `/api/v1/chats/{chatId}/messages/{messageId}` | 메시지 수정 (자동 분기) | ✓ | FR-1.5 |

총 9개 엔드포인트. Phase 2의 11개와 합쳐 누적 20개.

Phase 2 `DELETE /chats/{chatId}`는 Phase 3에서 cascade 처리가 추가되어 동작이 확장된다 (경로는 동일).
